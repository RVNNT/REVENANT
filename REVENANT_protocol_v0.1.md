...
**Principle:** everything deploys from a physical medium (USB / SSD / HDD).
Each level has its own offline installers, full stack, and documentation.

---

## Naming

**R (REVENANT stage)** — civilizational infrastructure deployment level:
```
R0 → Clan      (50–200 people)
R1 → Hex       (up to 25,000)
R2 → Polis     (up to 500,000)
R3 → Megalopolis (up to 1.2M)
```

**T (Transport layer)** — network connectivity level:
```
T0 → Clan mesh      (LoRa / phones)
T1 → Hex            (fiber backbone + wireless)
T2 → Polis          (fiber between Hex nodes, BGP)
T3 → Megalopolis    (metro optical rings, 5G/6G)
```
...
**Network: T2**
- Fiber between Hex nodes
- BGP, EVPN / MPLS
- ≥10 Mbps / subscriber
- Unifies multiple T1 networks

**Stack:**
- Multi-DC, multi-site storage
- PostgreSQL sharding (Citus)
- Kafka backbone, Elastic cluster
- Full e-government (Tax, Land registry, Court system)
- Redundant SCADA + Segmented OT + Air-gapped safety layer
- Full PKI + Hardware root of trust
- Full Nix/Guix reproducibility model (~40–80 GB)
- SOC (Security Operations Center) stack
- Offline cold backup repos / images / mirrors
...
- Metro optical rings
- 5G / 6G
- Unifies multiple T2 networks

**Stack:**
- Deep redundancy, active-active DC
- Strategic-scale PKI
- Scientific clusters
...
- Linux as the base
- Containers instead of custom OS
- Mirror only what is needed
- Stateless compute + replicated state
- Full offline reproducibility
- Documentation always included
- WireGuard + PKI at every level
- Deterministic builds: every binary is verifiable from source
- Checksum governance: medium = trusted artifact
- Defence in depth: network + host + physical + procedures
- Each Rn is autonomous and can operate without Rn+1

---

## Final Balance v0.1.1

| Level | Network | Medium | Real Volume |
|-------|---------|--------|-------------|
| R0 — Clan | T0 | 64–128 GB USB | ~33–52 GB |
| R1 — Hex | T1 | 1 TB SSD | ~321–493 GB |
| R2 — Polis | T2 | Multi-TB storage | scales |
| R3 — Megalopolis | T3 | Multi-DC | state scale |
...
### v0.1.1
- R0: Added Dev Environment Layer (~5–8 GB)
  - Shell tools: zsh, tmux, fzf, ripgrep, fd
  - Editors: Zed (primary) + Neovim (fallback) with full LSP stack
  - LSP: clangd, rust-analyzer, tsserver, shellcheck, markdownlint
  - Debug: lldb, cgdb, rust-gdb, valgrind, strace, ltrace + core dump policy
  - Build UX: meson, ccache, sccache, npm offline cache
  - Git: Forgejo local server, git-lfs, pre-commit hooks
  - Testing: pytest, criterion, gcov/lcov, tarpaulin
  - DevDocs mirror (minimal: C, Rust, Node, Bash, POSIX, ELF, SQLite, OpenSSL)
  - Containers: podman + buildah
  - Dev Security: cargo audit, clang-tidy, bandit, SSH CA
  - Project templates: C, Rust, embedded, CLI, Node
  - Dev Dashboard: r0 status CLI panel
  - Dev Backup: restic with auto-snapshot
  - Comfort: Fira Code font, Neovim + terminal color schemes

### v0.1.0
- Full English translation
- Subtitle: "from a flash drive to an industrial city"
- Version reset to 0.1.0

### v0.1.4 (Ukrainian)
- R0: Power Stack, Radio Doctrine, Metric Seed
- R1: Scavenging & Salvage Registry

### v0.1.3 (Ukrainian)
- Naming: C→R (stages), C→T (network layers)
- R0→R1 transfers: OpenEMR, FarmOS, Moodle, LinuxCNC

### v0.1.2 (Ukrainian)
- Electronics Stack → R1
- Hardware & Firmware Archive, Reproducible Build Pipeline
- Security Hardening Layer, Checksum Governance Layer

### v0.1.1 (Ukrainian)
- Medical, Agriculture, Fabrication, Education, Economy (R0)
- Kiwix + Wikipedia (R1)
